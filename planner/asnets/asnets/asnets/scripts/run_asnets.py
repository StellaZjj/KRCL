...
def eval_single(args, policy, problem_server, unique_prefix, elapsed_time,
    ...
    with open(actions_path, 'w') as fp:
        for alist in paths:
            fp.write(' -> '.join(alist))
            fp.write('\n\n')
    return goal_reached_times, mean_exec_times, mean_cost
...
def make_policy(args,
    ...
    # args.resume_from = '.../snapshots/snapshot_final.pkl'
...
def main_supervised(args, unique_prefix, snapshot_dir, scratch_dir):
    ...
    with ExitStack() as stack:
        ...
        if not args.no_train:
            ...
            train_gen = sup_trainer.train(max_epochs=200)
            best_rate = None
            iter_num = 0
            time_since_best = 0
        ...
        for problem in problems: 
            ...
            #weight_manager = open('experiment-results/P[domain,task01,task02,task03,task04,task25,task2...]-S[0.0005,128,lm-cut]-MO[]-T[inf]-03348078-474e1e4/snapshots/snapshot_final.pkl', 'rb')
            #weight_manager = pickle.load(weight_manager)
            #weight_manager = joblib.load('experiment-results/P[domain,task01,task02,task03,task04,task25,task2...]-S[0.0005,128,lm-cut]-MO[]-T[inf]-03348078-474e1e4/snapshots/snapshot_final.pkl')
            #weight_manager = joblib.load(args.resume_from)
...
def _main():
    ...
    sys.stdout = Logger('.../log/log_%r.log' % unique_prefix, sys.stdout)
    sys.stderr = Logger('.../log/log_%r.log' % unique_prefix, sys.stderr)
