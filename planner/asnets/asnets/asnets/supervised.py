...
from asnets.recognizer import call_recognizer
# import asnets.recognizer
...
@can_profile
def collect_paths(i, planner_exts, get_action, n_paths, max_len):
    ...
    for _ in range(n_paths):
        ...
        # max_len决定从当前（初始）状态往后走几步
        
        if os.path.exists('.../train_data_rec/action_p0{}.dat'.format(i)):
            os.remove('.../train_data_rec/action_p0{}.dat'.format(i))
        ...
        for _ in range(max_len):
            ...
            #a = prob_meta.bound_acts_ordered[action]        
            # 将每个问题的action序列记录在文件中
            with open('.../action_p0{}.dat'.format(i), 'a') as f:
                f.write(str(prob_meta.bound_acts_ordered[action]))
                f.write("\n")
    ...
    # 修改action文件格式
    content=open('.../train_data_rec/action_p0{}.dat'.format(i))
    with open('.../train_data_rec/obs_p0{}.dat'.format(i),'w') as f:
        for line in content:
            f.write('(')
            f.write(line.replace('Action ', '').replace(',', '').replace('(', ' '))
            #f.write(line.replace(',', ''))

            


    domain = planner_exts.domain_meta
    problem = planner_exts.problem_meta
    h1 = problem.num_acts + problem.num_props + len(problem.goal_props)
    h1 = (h1-0)/(105-0)
    h = h1
 
    
    with open('.../asnets-test/hardness.txt', 'a') as f:
        f.write(str(h))
        f.write("\n")    
...
def make_problem_service(config, set_proc_title=False):
    ...
    class ProblemService(rpyc.Service):
        ...
        def exposed_extend_replay(self, i, get_action, n_paths, no_plan=False):
            ...
            return self.internal_extend_replay(
                 i, get_action, n_paths, no_plan=no_plan)
        ...
        def internal_extend_replay(self, i, get_action, n_paths, no_plan=False):
...
class SupervisedTrainer:
    ...
    def train(self, *, max_epochs):
        ...
        if os.path.exists('.../loss/mean_loss_%r.txt' % max_epochs):
            os.remove('.../loss/mean_loss_%r.txt' % max_epochs)

        if os.path.exists('.../loss/mean_loss_recg_%r.txt' % max_epochs):
            os.remove('.../loss/mean_loss_recg_%r.txt' % max_epochs)
        
        if os.path.exists('.../loss/mean_total_loss_%r.txt' % max_epochs):
            os.remove('.../loss/mean_total_loss_%r.txt' % max_epochs)        
        
        # 记录所有epoch的loss
        loss_list = []
        list_asnets_mean_loss = []
        list_total_recg_loss = []
        list_mean_recg_loss = []
        list_mean_recg_loss_b = []
        list_mean_total_loss = []
        
        for epoch_num in tr:
            if os.path.exists('.../hardness.txt'):
                os.remove('.../hardness.txt')
        
            succs_probs, mean_recg_loss, total_recg_loss = self._extend_replays(1)
            ...
            if loss_list[epoch_num] > loss_list[epoch_num-1]:
                b = (loss_list[epoch_num] - loss_list[epoch_num-1])

            with open('.../loss/mean_loss_%r.txt' % max_epochs, 'a') as f:
                f.write(str(mean_loss))
                f.write("\n")

            list_asnets_mean_loss.append(mean_loss)


            mean_loss = mean_loss + total_recg_loss + mean_recg_loss*(b)          
            

            list_total_recg_loss.append(total_recg_loss)
            list_mean_recg_loss.append(mean_recg_loss)
            list_mean_recg_loss_b.append(mean_recg_loss*(b))
       
            
            with open('.../loss/mean_loss_recg_%r.txt' % max_epochs, 'a') as f:
                f.write(str(mean_loss))
                f.write("\n")        
            
            list_mean_total_loss.append(mean_loss)
            
            with open('.../loss/mean_total_loss_%r.txt' % max_epochs, 'a') as f:
                f.write(str(mean_loss))
                f.write("\n")                        
    ...
    def _extend_replays(self, num_per_problem):
        ...
        for problem in tqdm.tqdm(self.problems, desc='spawn extend'):
            ...
            if i<10:
                domain='.../train_data_rec/domain.pddl'
                problem1='.../train_data_rec/template_p0{}.pddl'.format(i)
                hyp='.../train_data_rec/hyps.dat'
                #obs='.../obs_p01.dat'
                real_hyp='.../train_data_rec/real_hyp_p0{}.dat'.format(i)
                obs='.../train_data_rec/obs_p0{}.dat'.format(i)
         
            else:
                domain='.../train_data_rec/domain.pddl'
                problem1='.../train_data_rec/template_p{}.pddl'.format(i)
                hyp='.../train_data_rec/hyps.dat'
                #obs='.../obs_p01.dat'
                real_hyp='.../train_data_rec/real_hyp_p{}.dat'.format(i)
                obs='.../train_data_rec/obs_p0{}.dat'.format(i)



           
            goal_sim = call_recognizer(domain, problem1, hyp, obs, real_hyp)
            recg_loss = 1 - goal_sim

            recg_loss_all.append(recg_loss)
        ...
        total_recg_loss = 0
        with open('.../hardness.txt', 'r') as f:
            h = f.readlines()      
            for i in range(len(self.problems)):
                if float(h[i]) >= 0.5:
                    total_recg_loss += recg_loss_all[i] * (float(h[i]))
                else:
                    total_recg_loss += recg_loss_all[i] * 0.0

            
            total_recg_loss = total_recg_loss/len(self.problems) 
            mean_recg_loss = sum(recg_loss_all)/len(self.problems) 

        return succ_rates, mean_recg_loss, total_recg_loss
