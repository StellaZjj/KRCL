...
def experiment_loop(agent: agent.Agent, saver: Saver, train_env: PDDLEnv, num_steps: int,
    ...
    with open('.../index.dat', 'rb') as file:
        index = int(file.readline()) + 1

    
    if os.path.exists('.../train_data_rec/obs_p0{}.dat'.format(index)):
        os.remove('.../train_data_rec/obs_p0{}.dat'.format(index))
    ...
    try:
        while t < num_steps:
            ...
            with open('.../train_data_rec/obs_p0{}.dat'.format(index), 'a') as f:
                f.write(str(a_t))
                f.write("\n")   
